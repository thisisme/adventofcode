...
input.forEach(bag => {
  ...
  line.forEach((element, i) => {
    if (i > 0) {
      tmp.push(element.replace(/.*\d /, "").replace(/[^a-zA-Z ]/g, "").replace(/(bags|bag)/,"").trim())
    }
  })
  bags[key] = tmp
})

const checkColor = (bags, bag, color) => {
  if (!bags[bag]) {
    return false
  }
  if (bags[bag].includes(color)) {
    return true
  }
  let control = false
  for (const innerBag of bags[bag]) {
    if (checkColor(bags, innerBag, color)) {
      control = true
    }
  }
...
for (const bag in bags) {
  total += checkColor(bags, bag, 'shiny gold') ? 1 : 0
}
