...
let bags = {}
input.forEach(bag => {
  const line = bag.replace('contain ', ',').split(',')
  const key = line[0].replace('bags', '').trim()
  let tmp = []
  line.forEach((element, i) => {
    if (i > 0) {
      tmp.push([
        Number(element.replace('.', '').trim().replace(/[^\d]+/g, '')),
        element.trim().replace(/[^a-zA-Z ]/g, '').replace(/(bags|bag)/, '').trim()
      ])
    }
  })
  bags[key] = tmp
...
const countBags = (bags, color) => {
  if (!bags[color]) {
    return 0
  }
  let innerBags = 0
  for (const innerBag of bags[color]) {
    innerBags += innerBag[0] + innerBag[0] * countBags(bags, innerBag[1])
  }
