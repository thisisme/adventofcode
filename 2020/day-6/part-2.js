...
const groups = input.split(/\n\n/).map(i => i.split(/\s/).map(c => c.split('')))
let tmp = []
groups.forEach(group => {
  let answered = []
  const groupLength = group.length
  const charLength = group[0].length
  for (let i = 0; i < charLength; i += 1) {
    const c = group[0][i]
    let check = true
    for (let j = 1; j < groupLength; j += 1) {
      const g = group[j]
      if (!g.includes(c) && check) {
        check = false
      }
    }
    if (check) {
  ...
  tmp.push(answered.length)
})

console.log(tmp.reduce((acc, t) => acc + t, 0))
